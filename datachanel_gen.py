...
ch_sequence = ["Right to Left","Left to Right"]
...
def get_ch_seq(event):
    if (ch_seq_combo.get()=="Left to Right"):
...
bit_num_i.bind('<FocusIn>', x1y1_guide)
                    ...
                    if (cell_val != None):
                        if(str(cell_val).find("BP_") != -1):
                            ...
                            if(index[0] != "NA"):
                                ...
                                wso_f[get_column_letter(c)+str(r)].value = str(cell_val).replace(index[1],'') + "[" + str(ch_cnt * bit_cnt + int(index[0])) + "]"
                            else:
                                wso_f[get_column_letter(c)+str(r)].value = str(cell_val) + "[" + str(ch_cnt) + "]"
                        else:
