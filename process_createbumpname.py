...
def process_index(s: str):
    m = re.search(r"\d", s)
    isfound = False
    if m:            
        print(f"first index:{m.start()}")
        print(f"end index:{m.end()}")
        s2 = s[:m.start()]+"[" + s[m.end()- 1:]+"]"
        print(s2)
        isfound = True
        return isfound, s2
    else:
        print("Not found any digit")
        isfound = False
        return isfound, None
...
excel_path = r'C:\Users\sytung\OneDrive - Synopsys, Inc\Documents\PKG\UCIe__Bumpmap_Implementation_Note_update.xlsx'

ref_map_range = "G14:N48"
ref_ws_name = 'UCIeA_x32_8col'

out_map_loc = "S14"
out_ws_name = "UCIeA_x32_8col"
...
if option == "Yes":
    ...
    for row in range(ref_row_begin, ref_row_end+ 1):
        for col in range(ref_col_begin, ref_col_end + 1):
            cellVal = ref_ws.cell(row=row, column=col).value
            if(cellVal != None):
                cellVal = str(cellVal).upper()
                if cellVal == 'VCCFWDIO':
                    cellVal = 'VDD'
                elif cellVal.find("RX") != -1 or cellVal.find("TX") != -1:
                    cellVal = "BP_" + cellVal
                    getindex = process_index(cellVal)
                    if getindex[0] == True:
                        cellVal = getindex[1]
                out_ws.cell(row=out_r, column=out_c).value = cellVal
            out_c += 1
        out_r += 1
        out_c = out_col_begin
wb.save(excel_path)